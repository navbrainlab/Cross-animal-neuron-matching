...
case "$action" in
  prepare)
    "$python_bin" "$root/baselines/official/adapters/gwot_md/prepare_atanas21_paper_inputs.py" \
      --h5-dir "$h5_dir" \
      --labels "$labels" \
  ...
  self-check)
    ...
    ;;
  official-h0-audit)
    "$python_bin" "$root/baselines/official/adapters/gwot_md/audit_gwtune_h0_equivalence.py" \
      --data-dir "$data_dir" \

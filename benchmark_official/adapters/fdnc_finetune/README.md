...
`benchmark_official/third_party/fdnc_official/src/model.py`

Initialization is the authors' downloaded:

`benchmark_official/third_party/fdnc_official/model/model.bin`
...
CUDA_VISIBLE_DEVICES=0 python -u \
  benchmark_official/adapters/fdnc_finetune/train_fdnc_official_finetune.py \
  --dataset atanas \
  --fold 1 \
  --seed 42 \
  --save-dir benchmark_official/runs/fdnc_smoke \
  --unfreeze-last-n 2 \
  --backbone-lr 5e-6 \
  --epochs 2 \
  --pairs-per-epoch 10 \
...
```bash
rm -rf benchmark_official/runs/fdnc_smoke

GPUS="0 1" bash \
  benchmark_official/adapters/fdnc_finetune/run_fdnc_official_finetune_cv5x3.sh
```

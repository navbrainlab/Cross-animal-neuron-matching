...
BASE="$ROOT/benchmark_official/runs/fdnc_official_finetuned"
...
EVAL="$ROOT/benchmark_official/adapters/fdnc_finetune/evaluate_fdnc_official_finetuned.py"

cd "$ROOT" || exit 1
mkdir -p "$BASE" "$ROOT/benchmark_official/logs"
...
launch_candidate () {
  ...
  log="$ROOT/benchmark_official/logs/fdnc_ft_${dataset}_f${fold}_s${seed}_${tag}.log"
    ...
    for seed in 1 42 123; do
      ...
      if [ -f "$testdir/result.json" ]; then
        ...
        CUDA_VISIBLE_DEVICES="$gpu" "$PYTHON" -u "$EVAL" \
          ...
          --precision fp32 \
          --device cuda:0 \
          2>&1 | tee "$ROOT/benchmark_official/logs/fdnc_test_medoid_template_v1_${dataset}_f${fold}_s${seed}.log"
      fi
    done
  done
done
...
root=Path("/home/ubuntu/klb/nuclr/nuclr/benchmark_official/runs/fdnc_official_finetuned")
out=Path("/home/ubuntu/klb/nuclr/nuclr/benchmark_official/results")

...
`NeuRID` is the public model name. The Python package and original class name
remain `mprt_net` and `MPRTNet` so existing checkpoints and experiment scripts
continue to load unchanged. New code may use the equivalent `NeuRID` alias:

```python
from mprt_net import NeuRID
```

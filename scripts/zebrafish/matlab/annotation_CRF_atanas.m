...
function [landmark_match_score,not_match,node_label,Neuron_head,conserved_nodeBel] = annotation_CRF_atanas(data_file,numLandmarks,numLabelRemove,rng_fix_landmark,num,atlas_file)
rng shuffle
%%% Atanas adapter: explicit data and atlas files
load(data_file);
load(atlas_file);

%%% Atanas adapter: retain complete CRF atlas vocabulary
% No AML5-specific keep_neurons filtering.
...
% Atanas adapter: label-free fixed XYZ coordinate convention.
% No test neuron identities are used for axis orientation.
mu_r_centered = mu_marker - repmat(mean(mu_marker,1),size(mu_marker,1),1);
...
% UGM path supplied by Atanas driver
...
for i = 1:edgeStruct.nEdges
    ...
    % Atanas numerical guard for degenerate spatial edges.
        % If two query neurons have identical coordinates, their relative
        % direction is undefined. Use neutral angular evidence for this edge
        % rather than allowing NaN values to enter UGM-LBP.
        query_vec = [ ...
            X(node2,1)-X(node1,1), ...
            Y(node2,1)-Y(node1,1), ...
            Z(node2,1)-Z(node1,1) ...
        ...
        if norm(query_vec) < 1e-12
            angle_matrix = zeros(size(X_rot,1),size(X_rot,1));
        else
            angle_matrix = get_relative_angles( ...
                X_rot,Y_rot,Z_rot,X,Y,Z,node1,node2 ...
            );

            % Extra floating-point protection for acos/division roundoff.
            angle_matrix(~isfinite(angle_matrix)) = 0;
        end
